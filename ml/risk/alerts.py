...
def assess_market_volatility(
    forecast_best, forecast_expected, forecast_worst, recent_volatility=None
):
...
def assess_congestion(load_port, discharge_port, load_congestion, discharge_congestion):
...
def assess_decision_risk(net_expected_saving, waiting_cost, probability_waiting_wins=None):
    ...
    if probability_waiting_wins is not None and 0.42 <= probability_waiting_wins <= 0.58:
        alerts.append(
            _alert(
                "decision",
                "medium",
...
def build_risk_report(
    ...
    alerts += assess_congestion(load_port, discharge_port, load_congestion, discharge_congestion)

    alerts += assess_supply(vessel_supply, available_vessels)

    alerts += assess_decision_risk(net_expected_saving, waiting_cost, probability_waiting_wins)
