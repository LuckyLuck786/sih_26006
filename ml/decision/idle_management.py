...
def analyse_idle(
    ...
    load_wait = estimate_berth_wait_days(load_congestion, load_port.get("berths", 2))

    discharge_wait = estimate_berth_wait_days(discharge_congestion, discharge_port.get("berths", 2))

    # Laden out plus ballast back.
    ballast_days = sea_days

    total_days = sea_days + ballast_days + load_days + discharge_days + load_wait + discharge_wait
        ...
        recommendations.append(
            {
                ...
                "severity": "low",
                "message": (
                    "No material idle-time exposure on this lane at current congestion levels."
                ),
            }
        )
...
if __name__ == "__main__":
    ...
    for discharge, congestion in [("Gangavaram", 0.25), ("Haldia", 0.72)]:
        ...
        print(f"  idle           : {result['idle_days']} days ({result['idle_share'] * 100:.0f}%)")
