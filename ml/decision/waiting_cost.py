...
def calculate_waiting_cost(
    ...
    cargo_value = cargo_quantity * cargo_value_per_ton

    # -----------------------------------------
    # 2. Daily carrying cost
    # -----------------------------------------

    daily_carrying_cost = cargo_value * annual_carrying_rate / 365

    # -----------------------------------------
    # 3. Total waiting cost
    # -----------------------------------------

    total_waiting_cost = daily_carrying_cost + storage_cost_per_day + operational_delay_cost_per_day

    return {
        "cargo_value": round(cargo_value, 2),
        "daily_carrying_cost": round(daily_carrying_cost, 2),
        "storage_cost_per_day": round(storage_cost_per_day, 2),
        "operational_delay_cost_per_day": round(operational_delay_cost_per_day, 2),
        "waiting_cost_per_day": round(total_waiting_cost, 2),
    }
...
if __name__ == "__main__":
    ...
    print(f"\nTotal cargo value: ₹{result['cargo_value']:,.2f}")

    print(f"\nDaily carrying cost: ₹{result['daily_carrying_cost']:,.2f}")

    print(f"\nStorage cost/day: ₹{result['storage_cost_per_day']:,.2f}")

    print(f"\nOperational delay cost/day: ₹{result['operational_delay_cost_per_day']:,.2f}")

    print("\n----------------------------------------")

    print(f"\nTOTAL WAITING COST/DAY: ₹{result['waiting_cost_per_day']:,.2f}")
