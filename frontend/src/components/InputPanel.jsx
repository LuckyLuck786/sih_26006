...
// actually exist. Both now cover every location named in PS 26006,
// including Sagar-Sandheads, which the first prototype omitted.
const selectOptions = {
  origin: ORIGINS,
...
export default function InputPanel({ values, onChange, onSubmit, loading }) {
  ...
  const validate = () => {
    ...
    ;['origin', 'destination', 'cargo_type', 'cargo_quantity', 'contract_duration'].forEach((field) => {
      if (!String(values[field] || '').trim()) nextErrors[field] = 'This field is required.'
    })

    ;['cargo_quantity', 'contract_duration', 'cargo_value_per_ton', 'annual_carrying_rate'].forEach((field) => {
      ...
      <div className="space-y-4">
        {['origin', 'destination', 'cargo_type', 'vessel_class'].map((name) => (
          <div key={name}>
            ...
            >
              <option value="">
                {name === 'vessel_class' ? 'Auto — let the optimiser choose' : `Select ${LABELS[name].toLowerCase()}`}
              </option>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {errors.contract_duration && <p className="mt-1 text-xs text-red-600">{errors.contract_duration}</p>}
          </div>
        </div>
      </div>
      ...
      {advancedOpen && (
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          <p className="col-span-2 text-xs leading-5 text-slate-400">
            These set the cost of waiting: cargo value multiplied by the carrying rate gives the daily
            capital cost of holding the position open.
          </p>
        </div>
      )}
